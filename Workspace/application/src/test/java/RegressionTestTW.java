...
public class HealthCheckTW {
  ...
  public void LoginTest() throws Exception {
    ...
    driver
        .findElement(
            By.xpath("//table[@id='Main']/tbody/tr[3]/td/table/tbody/tr[2]/td[4]/table/tbody/tr[3]/td[2]/a/span"))
        .click();
    wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//td[2]/input")));
    URL = driver.getCurrentUrl();
    assertTrue(URL.contains(
        "https://application.company.com/portalWeb/com/asis/company/useradmin/portlet/CustLogin/toLoginPage.do?COMPANY_CD=7700"),
        "Taiwan Login Page Fail");
  ...
  public void RetrievePassword() throws Exception {
    ...
    URL = driver.getCurrentUrl();
    assertTrue(URL.contains(
        "https://application.company.com/portalWeb/com/asis/company/useradmin/portlet/CustLogin/CustLoginController.jpf"),
        "Logout Fail");
    ...
    URL = driver.getCurrentUrl();
    assertTrue(URL.contains(
        "https://application.company.com/portalWeb/com/asis/company/useradmin/portlet/forgotpwd/begin.do?companyCd=7700"),
    ...
    assertTrue(TextOnPage.contains("Your Password has sent to"), "Retrieve Password Fail");
  }
  ...
  public void TradeMeetingMenu(@Optional("Switch") String SW, String TM) throws Exception {
    ...
    if (SW.equals("2023")) {
      driver.findElement(By.id("switchIcon2023")).click();
    }
    if (SW.equals("2022")) {
      driver.findElement(By.id("switchIcon2022")).click();
    }
    if (SW.equals("2021")) {
      driver.findElement(By.id("switchIcon2021")).click();
    }
  ...
  public void SalesOrder() throws Exception {
    ...
    while (iterator.hasNext()) {
      ...
      if (!mainWindowHandle.equalsIgnoreCase(ChildWindow)) {
        driver.switchTo().window(ChildWindow);
        Filename = driver.findElement(By.xpath("//a[contains(text(),'SO_7700')]")).getText();
        driver.findElement(By.linkText(Filename)).click();
        boolean check = new File(DownloadFolder, Filename).exists();
        while (!check) {
          check = new File(DownloadFolder, Filename).exists();
  ...
  @Test(priority = 9, enabled = false) // No Data
  public void FinanceDocument() throws Exception {
    ...
    while (iterator.hasNext()) {
      ...
      if (!mainWindowHandle.equalsIgnoreCase(ChildWindow)) {
        driver.switchTo().window(ChildWindow);
        Filename = driver.findElement(By.xpath("//a[contains(text(),'IN_7700')]")).getText();
        driver.findElement(By.linkText(Filename)).click();
        boolean check = new File(DownloadFolder, Filename).exists();
        while (!check) {
          check = new File(DownloadFolder, Filename).exists();
