...
public class HealthCheckKR {
  ...
  public void LoginTest() throws Exception {
    ...
    driver
        .findElement(
            By.xpath("//table[@id='Main']/tbody/tr[3]/td/table/tbody/tr[2]/td[4]/table/tbody/tr/td[4]/a/span"))
        .click();
    wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//td[2]/input")));
    URL = driver.getCurrentUrl();
    assertTrue(URL.contains(
        "https://application.company.com/portalWeb/com/asis/application/useradmin/portlet/CustLogin/toLoginPage.do?COMPANY_CD=7800"),
        "Korea Login Page Fail");
  ...
  public void RetrievePassword() throws Exception {
    ...
    driver
        .findElement(
            By.xpath("//table[@id='Main']/tbody/tr[3]/td/table/tbody/tr[2]/td[4]/table/tbody/tr/td[4]/a/span"))
        .click();
    wait.until(ExpectedConditions.elementToBeClickable(By.linkText("비밀번호 찾기?")));
    ...
    URL = driver.getCurrentUrl();
    assertTrue(URL.contains(
        "https://application.company.com/portalWeb/com/asis/application/useradmin/portlet/forgotpwd/begin.do?companyCd=7800"),
    ...
    assertTrue(TextOnPage.contains("암호가" + Email + "으로 전송되었습니다"), "Retrieve Password Fail");
  }
  ...
  public void TradeMeetingMenu(@Optional("Switch") String SW, String TM) throws Exception {
    ...
    wait.until(ExpectedConditions.elementToBeClickable(By.linkText("Printable Catalogue")));
  ...
  public void SalesOrder() throws Exception {
    ...
    while (iterator.hasNext()) {
      ...
      if (!mainWindowHandle.equalsIgnoreCase(ChildWindow)) {
        driver.switchTo().window(ChildWindow);
        Filename = driver.findElement(By.xpath("//a[contains(text(),'SO_78')]")).getText();
        driver.findElement(By.linkText(Filename)).click();
        boolean check = new File(DownloadFolder, Filename).exists();
        while (!check) {
          check = new File(DownloadFolder, Filename).exists();
  ...
  public void FinanceDocument() throws Exception {
    ...
    while (iterator.hasNext()) {
      ...
      if (!mainWindowHandle.equalsIgnoreCase(ChildWindow)) {
        driver.switchTo().window(ChildWindow);
        Filename = driver.findElement(By.xpath("//a[contains(text(),'CS_7800')]")).getText();
        driver.findElement(By.linkText(Filename)).click();
        boolean check = new File(DownloadFolder, Filename).exists();
        while (!check) {
          check = new File(DownloadFolder, Filename).exists();
